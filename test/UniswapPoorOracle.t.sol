...
contract UniswapPoorOracleTest is Test, UniswapDeployer {
    ...
    function test_outOfRangeRecording_outOfRangeAllTheTime() public {
        ...
        tokenA.mint(address(this), 1e18);
        tokenB.mint(address(this), 1e18);
        BunniKey memory k = BunniKey({pool: pool, tickLower: tickLower, tickUpper: TICK_UPPER});
        ...
        // record
        oracle.startRecording(address(pool), 100, TICK_UPPER);
        skip(RECORDING_MIN_LENGTH);
        UniswapPoorOracle.PositionState state = oracle.finishRecording(address(pool), 100, TICK_UPPER);
